...
def create_folder(folder_name):
    if not os.path.exists('./data2/training_set/' + folder_name):
        os.makedirs('./data2/training_set/' + folder_name)
    if not os.path.exists('./data2/test_set/' + folder_name):
        os.makedirs('./data2/test_set/' + folder_name)
...
def capture_images(ges_name):
    ...
    cv2.namedWindow("image_acquistion")
    ...
    for loop in listImage:
        while True:
            ...
            if cv2.waitKey(1) == ord('m'):
                ...
                if t_counter <= 250:
                    img_name = "./data2/training_set/" + str(ges_name) + "/{}.png".format(training_set_image_name)
                ...
                if t_counter > 250 and t_counter <= 500:
                    img_name = "./data2/test_set/" + str(ges_name) + "/{}.png".format(test_set_image_name)
                    save_img = cv2.resize(mask, (image_x, image_y))
                    cv2.imwrite(img_name, save_img)
                    print("{} written!".format(img_name))
                    test_set_image_name += 1
